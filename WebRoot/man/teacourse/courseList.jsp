	...
	<head>
		...
		<script language="javascript">
           function courseDel(id) {
               if(confirm('您确定删除吗？')){
                   window.location.href="<%=path%>/teacourseDel.action?id="+id;
               }
           }
       
           function courseAdd(){
           		var url="<%=path%>/teacourseToAdd.action";
					...
					<table class="man-table" style=" table-layout: fixed; ">
						...
						<tbody>
							<c:forEach items="${courseList}" var="course" varStatus="status">
								...
								<tr>
									<td>${course.xmname} </td>
									<td style=" white-space:nowrap;overflow:hidden;text-overflow: ellipsis;" target="_blank"title="${course.name}">${course.name} </td>
									<td>${course.time}</td>
									<td>${course.jiage}</td>
